...
- `--save-normalized-dir`: Optional directory where normalized page images are written for inspection before or alongside the PDF result. This does not enable OCR and does not change the final PDF mode.
...
- `--ocr`: Runs Tesseract OCR and writes a searchable PDF with a text layer instead of an image-only PDF. Without this flag, output remains image-only.
